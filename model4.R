...
doLoad<-function() {
  ...
  for(i in 2:6 ) {
    gc()
    ng<-quanteda::tokenize(text_string,ngrams=i,concatenator=" ", remove_punct=TRUE, remove_numbers=TRUE,remove_symbols=TRUE)
    print("tokenized")
    myDfm<-dfm(ng)
    x<-colSums(myDfm)
    totLst[[i-1]]<-sum(x)
